...
export function MediaCard({ media, currentUserId, isEventCreator, userRole }: MediaCardProps) {
  ...
  return (
    <>
      <div className={`break-inside-avoid rounded-xl overflow-hidden bg-background/5 border border-white/10 group relative mb-4 shadow-xl ${mediaError ? 'hidden' : ''}`}>
        ...
        {/* Interaction Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition duration-300 flex flex-col justify-end p-4 pointer-events-none">
          <div className="flex items-center justify-between pointer-events-auto w-full">
            <div className="flex space-x-3">
              <button 
                onClick={handleLike}
                disabled={isLiking}
                className="flex items-center space-x-1.5 text-foreground hover:text-pink-400 transition"
              >
              ...
              <button 
                ...
                className="flex items-center space-x-1.5 text-foreground hover:text-[#8FAD88] transition"
              >
        ...
        {showLightbox && (
          ...
          >
            <div className="absolute top-6 right-6 flex items-center space-x-3 z-50">
              {canDownload && (
                <button 
                  className="text-foreground/70 hover:text-foreground transition bg-background/10 hover:bg-background/50 p-2.5 rounded-full"
